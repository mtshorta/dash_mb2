...
if Arquivo:
    ...
    df['PRIMEIRA MSG DO ATENDENTE'] = df['PRIMEIRA MSG DO ATENDENTE'].replace('---', pd.NaT)
    df['PRIMEIRA MSG DO ATENDENTE'] = pd.to_datetime(df['PRIMEIRA MSG DO ATENDENTE'], format='%d/%m/%Y %H:%M:%S')
